...
contract FeeOnTransferERC20 {
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool) {
        ...
        // Only transfer 50% (fee-on-transfer)
...
contract ReentrantERC20 {
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool) {
        ...
        // Attempt reentrancy on first transfer (to owner for the fee)
        // We set hasReentered before the call so we can check it later
...
contract HighGasERC20 {
    mapping(address => uint256) public balanceOf;
    mapping(address => mapping(address => uint256)) public allowance;
    mapping(uint256 => uint256) public gasWaster; // Storage to waste gas
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool) {
        ...
        // Waste gas with storage writes (but stay under block limit)
...
contract ConditionalFailureERC20 {
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool) {
        ...
        // Fail if transferring to target address
